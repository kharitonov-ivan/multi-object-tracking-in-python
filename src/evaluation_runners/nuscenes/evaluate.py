...
class TrackingEval:
    ...
    def render(self, md_list: TrackingMetricDataList) -> None:
        ...
        for metric_name in LEGACY_METRICS:
            recall_metric_curve(md_list, metric_name, self.cfg.min_recall, savepath=savepath("%s" % metric_name))
...
if __name__ == "__main__":
    # Settings.
    parser = argparse.ArgumentParser(description="Evaluate nuScenes tracking results.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--render_curves", type=int, default=1, help="Whether to render statistic curves to disk.")
