...
class NuscenesTrackerEvaluator:
    ...
    def process_scene(self, scene_token: str) -> Dict:
        ...
        tracker = PMBM(
            ...
            motion_model=src.motion_models.ConstantVelocityMotionModel(dt, sigma_q=5.0),
            ...
            density=src.common.GaussianDensity,
        ...
        for timestep, sample_token in tqdm.tqdm(enumerate(scene_sample_tokens)):
            ...
            for annotation in annotations:
                if annotation["category_name"].split(".")[0] == "vehicle" and annotation["num_lidar_pts"] > 10:
                    ...
                    scene_data[object_id] = Gaussian(
                        means=np.array([object_pos_x, object_pos_y, 0.0, 0.0]),
                        covs=np.eye(4),
                    )

            evaluator.step(
                sample_measurements=measurements,
                sample_estimates=estimation,
                sample_gt=scene_data,
                timestep=timestep,
            )
            ...
            def format_nuscenes_sample_results(sample_token: str, tracker_estimations):
                if tracker_estimations:
                    ...
                    for tracker_estimation_kv in tracker_estimations:
                        ...
                        translation = [
                            list(tracker_estimation)[0],
                            list(tracker_estimation)[1],
                            0,
                        ]  # x, y, z
