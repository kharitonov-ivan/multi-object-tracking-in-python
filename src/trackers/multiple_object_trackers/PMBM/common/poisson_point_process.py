...
class PoissonRFS:
    ...
    def get_targets_detected_for_first_time(
        ...
        S = (
            H_x @ self.intensity.covs @ H_x_T + model_measurement.R
        )  # Innovation covariance
        S = make_SPD(S)  # Make sure matrix S is positive definite

        result, dists = GaussianDensity.ellipsoidal_gating(
            self.intensity, measurements, model_measurement, gating_size, H_x, S
        )
        ...
        for meas_idx, curr_mask in zip(range(len(measurements)), result.T):
            ...
            next_state_means, next_state_covs, H_X_S = GaussianDensity.update(
                GaussianDensity(
                    ...
                    self.intensity.weigths[curr_mask],
                ),
            ...
            log_likelihoods: tp.Annotated[
                np.ndarray, "n_gated_gaussians, n_measurements"
            ] = GaussianDensity.predict_loglikelihood(
                GaussianDensity(
                    self.intensity.means[curr_mask],
                    self.intensity.covs[curr_mask],
                    self.intensity.weigths_log[curr_mask],
                ),
                measurements[meas_idx][None, :],
                model_measurement,
                H_X_S,
            ...
            log_weights: tp.Annotated[
                np.ndarray, "N_gated_gaussians, N_measurements"
            ] = (
                log_likelihoods
                + self.intensity.weigths_log[curr_mask][:, None]
                + np.log(detection_probability)
            )  # same shape as loglikelihoods

            normalized_log_weights, log_sum = normalize_log_weights(
                log_weights, axis=-1
            )
            ...
            merged_means: tp.Annotated[
                np.ndarray, "N_measurements, State_dim"
            ] = np.zeros((len(measurements), self.intensity.means.shape[-1]))
            merged_covs: tp.Annotated[
                np.ndarray, "(n_measurements, state_dim, state_dim)"
            ] = np.zeros(
                (
                    len(measurements),
            ...
            merge_state = GaussianDensity.moment_matching(
                ...
                GaussianDensity(
                    next_state_means[:, 0, ...], next_state_covs[:, 0, ...]
                ),
            )
            ...
            merge_state = GaussianDensity(
                merge_state.means[None, ...], merge_state.covs[None, ...]
            )
    ...
    def undetected_update(self, detection_probability) -> None:
        ...
        self.intensity.weights = self.intensity.weights + np.log(
