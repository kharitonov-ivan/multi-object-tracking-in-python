...
class MeasurementData:
    ...
    def generate_observations(self):
        ...
        if len(objects_in_scene) > 0:
            ...
            if not observed_objects:
                return
            gt_state_means = np.array([obj.x for _, obj in observed_objects])
            object_observations = np.array([self.meas_model.observe(x) for x in gt_state_means])
    ...
    def __repr__(self) -> str:
        return self.__class__.__name__ + (f"(sensor_model={self.sensor_model}, " f"meas_model={self.meas_model}, " f"X={self._observed_measurements}, ")
