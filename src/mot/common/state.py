...
#         return f"{self.__class__.__name__} " f"x = {np.array2string(self.means, max_line_width=np.inf, precision =1)}"

#     def __eq__(self, other) -> bool:
#         assert isinstance(other, Gaussian)
#         if self.means.__eq__(other.means).all() and self.covs.__eq__(other.covs).all():
#             return True
#         else:
#             return False
...
#     def __repr__(self) -> str:
#         return f"\n" f"weighted Gaussian with: " f"w = {self.log_weight:.2f}   " f"x = {np.array2string(self.gaussian.means, max_line_width=np.inf, precision =2)}  "
...
#             self.means = np.expand_dims(self.means, axis=0)
#         if len(self.covs.shape) == 2:
#             self.covs = np.expand_dims(self.covs, axis=0)
#         if len(self.weigths_log.shape) == 0:
#             self.weigths_log = np.array([self.weigths_log])
...
#     @property
#     def states_np(self):
#         return np.array([state.gaussian.means for state in self.weighted_components])

#     @property
#     def covariances_np(self):
#         return np.array([state.gaussian.covs for state in self.weighted_components])
