...
class Test_MeasurementData(unittest.TestCase):
    def test_function_name(self):
        ...
        objects_configs = [
            Object(
                initial=Gaussian(x=np.array([0.0, 0.0, 10.0, 10.0]),
                                 P=np.eye(4)),
                t_birth=0,
                t_death=total_time,
            )
        ]
        ground_truth = GroundTruthConfig(n_births=n_births,
                                         object_configs=objects_configs,
        ...
        sensor_model = SensorModelConfig(P_D=P_D,
                                         lambda_c=lambda_c,
                                         range_c=range_c)
