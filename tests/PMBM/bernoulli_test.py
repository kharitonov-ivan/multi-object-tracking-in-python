...
def test_bern_predict(initial_bernoulli, cv_motion_model, P_S):
    ...
    np.testing.assert_allclose(r_ref,
                               initial_bernoulli.existence_probability,
                               rtol=0.05)
    np.testing.assert_allclose(state_ref.x,
                               initial_bernoulli.state.x,
                               atol=1e-4)
    np.testing.assert_allclose(state_ref.P,
                               initial_bernoulli.state.P,
                               atol=1e-4)
...
def test_bern_undetected_update(initial_bernoulli, P_D):
    ...
    np.testing.assert_allclose(new_bern.existence_probability,
                               ref_bern_r,
                               rtol=0.05)
    np.testing.assert_allclose(log_likelihood_undetected,
                               ref_lik_undetected,
                               rtol=0.05)


def test_bern_detected_update_likelihood_outlier(initial_bernoulli, P_D,
    ...
    np.testing.assert_allclose(likelihood_detected,
                               likelihood_detected_ref,
                               rtol=0.05)


def test_bern_detected_update_likelihood_target(initial_bernoulli, P_D,
                                                cv_measurement_model,
    ...
    np.testing.assert_allclose(log_likelihood_detected,
                               log_likelihood_detected_ref,
                               rtol=0.05)


def test_bern_update_state(initial_bernoulli, neighbour_measurement,
                           cv_measurement_model):

    new_bernoulli = Bernoulli.detected_update_state(initial_bernoulli,
                                                    neighbour_measurement,
                                                    cv_measurement_model)
    ...
    np.testing.assert_allclose(new_bernoulli.existence_probability,
                               ref_r,
                               rtol=0.01)
